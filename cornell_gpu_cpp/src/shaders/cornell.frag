...
//
// Geometria, materiales, luz y camara replican los valores CANONICOS de la
// Cornell Box de referencia (radiometric data de Cornell University, los
// mismos usados en https://github.com/yumcyaWiz/glsl330-cornellbox):
// caja de 556 x 548.8 x 559.2, reflectancias blanco 0.8 / rojo (0.8,0.05,0.05)
// / verde (0.05,0.8,0.05), luz rectangular con emision (34,19,10).
// ==========================================================================
...
// -------------------------------------------------------------------------
// Escena: caja canonica de Cornell (datos radiometricos de Cornell Univ.)
// -------------------------------------------------------------------------
const vec3 BOX_MIN = vec3(0.0, 0.0, 0.0);
const vec3 BOX_MAX = vec3(556.0, 548.8, 559.2);

const float LIGHT_Y = 548.6;
const float LIGHT_X0 = 213.0, LIGHT_X1 = 343.0;
const float LIGHT_Z0 = 227.0, LIGHT_Z1 = 332.0;
const vec3 LIGHT_EMISSION = vec3(34.0, 19.0, 10.0);
const float LIGHT_AREA = (LIGHT_X1 - LIGHT_X0) * (LIGHT_Z1 - LIGHT_Z0);

const vec3 SPHERE1_C = vec3(186.0, 100.0, 169.5);
const float SPHERE1_R = 100.0;
const vec3 SPHERE2_C = vec3(393.0, 120.0, 351.0);
const float SPHERE2_R = 120.0;

const float PI = 3.14159265359;
const float EPS = 0.5;
const float GLASS_IOR = 1.5;
const vec3 FIREFLY_CAP = vec3(300.0);
...
bool intersectLightQuad(vec3 ro, vec3 rd, out float t) {
    ...
    vec3 p = ro + rd * t;
    return p.x >= LIGHT_X0 && p.x <= LIGHT_X1 && p.z >= LIGHT_Z0 && p.z <= LIGHT_Z1;
}

// Paredes = interior de la caja AABB; la camara siempre esta dentro, asi
// que el impacto es la salida mas cercana (min de los "t lejanos").
// x=0 -> pared roja, x=556 -> pared verde (igual que la Cornell Box clasica).
bool intersectWalls(vec3 ro, vec3 rd, out float t, out vec3 n, out vec3 albedo) {
    vec3 invD = 1.0 / rd;
    vec3 t1 = (BOX_MIN - ro) * invD;
    vec3 t2 = (BOX_MAX - ro) * invD;
    vec3 tFar = max(t1, t2);
...
bool sceneIntersect(vec3 ro, vec3 rd, out Hit h) {
    ...
    if (intersectSphere(ro, rd, SPHERE1_C, SPHERE1_R, t) && t < bestT) {
        ...
        h.albedo = vec3(0.8, 0.8, 0.8); h.emission = vec3(0.0); h.matType = 0; h.shininess = 1.0;
    }
    if (intersectSphere(ro, rd, SPHERE2_C, SPHERE2_R, t) && t < bestT) {
        ...
        h.shininess = 1.0;
        h.albedo = (u_sphere2_material == 0) ? vec3(0.8, 0.8, 0.8)
...
// punto central, para que la penumbra se vea gradual en todos los modos.
vec3 directLight(vec3 p, vec3 n, vec3 albedo) {
    const int LSAMPLES = 4;
    vec2 offs[4] = vec2[4](vec2(0.0, 0.0), vec2(1.0, 0.0), vec2(0.0, 1.0), vec2(1.0, 1.0));
    vec3 Li = LIGHT_EMISSION * u_light_intensity;
    vec3 result = vec3(0.0);
    for (int i = 0; i < LSAMPLES; i++) {
        vec3 lp = vec3(mix(LIGHT_X0, LIGHT_X1, offs[i].x), LIGHT_Y, mix(LIGHT_Z0, LIGHT_Z1, offs[i].y));
        vec3 toL = lp - p;
        float dist2 = max(dot(toL, toL), 1.0);
    ...
    return min(result, FIREFLY_CAP);
}

vec3 phongSpecular(vec3 p, vec3 n, vec3 viewDir) {
    vec3 lightCenter = vec3(0.5 * (LIGHT_X0 + LIGHT_X1), LIGHT_Y, 0.5 * (LIGHT_Z0 + LIGHT_Z1));
    vec3 l = normalize(lightCenter - p);
    if (dot(n, l) <= 0.0) return vec3(0.0);
    if (occluded(p, n, lightCenter)) return vec3(0.0);
    vec3 r = reflect(-l, n);
    float spec = pow(max(dot(r, viewDir), 0.0), 56.0);
    return vec3(1.0, 0.98, 0.92) * spec * 0.6 * u_light_intensity;
}
...
const vec3 FILL_LIGHT = vec3(0.9, 0.82, 0.66);
...
vec3 shadeSurfaceLocal(vec3 viewDir, Hit h) {
    ...
    return directLight(h.p, h.n, albedo) + spec + u_ambient * albedo + FILL_LIGHT * albedo * 0.15;
}
...
// -------------------------------------------------------------------------
// Modo 0 -- LOCAL (Phong difuso + especular + ambiente, sin GI)
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
// Modo 1 -- RAY TRACING recursivo (Whitted, 1980)
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
// Modo 2 -- PATH TRACING progresivo (Kajiya, 1986): NEE + RR
// -------------------------------------------------------------------------
vec3 shadePathtrace(vec3 ro, vec3 rd) {
    ...
    for (int b = 0; b < u_max_bounces; b++) {
        ...
        if (h.matType == 2) {
            ...
            vec3 refrDir = refract(rd, n, eta);
            bool tir = (refrDir == vec3(0.0));
            rd = (tir || rand() < fresnel) ? reflect(rd, h.n) : refrDir;
            throughput *= 0.97;
            ro = h.p + rd * (2.0 * EPS);
            continue;
        }
        ...
        vec2 r = rand2();
        vec3 lp = vec3(mix(LIGHT_X0, LIGHT_X1, r.x), LIGHT_Y, mix(LIGHT_Z0, LIGHT_Z1, r.y));
        vec3 toL = lp - h.p;
        ...
        if (ndotl > 0.0 && ndotlLight > 0.02 && dist2 > 1.0) {
...
// -------------------------------------------------------------------------
// Modo 4 -- RADIOSIDAD (Goral et al., 1984): GI difusa determinista
// -------------------------------------------------------------------------
vec3 shadeRadiosity(vec3 ro, vec3 rd) {
    ...
    vec3 direct = directLight(p, n, albedo);
    ...
    for (int i = 0; i < NDIRS; i++) {
        ...
        if (sceneIntersect(iro, dir, ih) && ih.matType != 3) {
            ...
            indirect += albedo * directLight(ih.p, ih.n, ialbedo);
        }
    }
...
// -------------------------------------------------------------------------
// Modo 5 -- MAPEO DE FOTONES (aproximacion eficiente): vidrio + caustica
// -------------------------------------------------------------------------
vec3 causticApprox(vec3 p, vec3 n) {
    ...
    float width = SPHERE2_R * 0.27;
    float ring = exp(-pow((dist - ringR) / width, 2.0));
    float core = exp(-pow(dist / (SPHERE2_R * 0.32), 2.0)) * 0.5;
    return LIGHT_EMISSION * u_light_intensity * 0.55 * (ring + core) * vec3(1.0, 0.96, 0.85);
}
